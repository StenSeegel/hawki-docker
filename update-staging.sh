...
# Allows building the image while the old version is still running.
#
# Usage:
#   ./update-staging.sh [--build] [--update] [--init]
#
# Options:
#   --build    Rebuild Docker images (without stopping containers)
...
if [ "$DO_BUILD" = false ] && [ "$DO_UPDATE" = false ] && [ "$FORCE_INIT" = false ]; then
    echo "❌ Error: Please specify at least one action: --build, --update, or --init"
    echo "Usage: ./update-staging.sh [--build] [--update] [--init]"
    exit 1
fi
...
# Check if image exists, if not, force build during update
if ! docker image inspect "$PROJECT_HAWKI_IMAGE" >/dev/null 2>&1; then
    echo "📦 Image $PROJECT_HAWKI_IMAGE not found, building required..."
    DO_BUILD=true
fi
...
if [ "$DO_UPDATE" = true ]; then
    ...
    # Prepare proxy args for docker compose up
    if [ -n "$DOCKER_HTTP_PROXY" ]; then
